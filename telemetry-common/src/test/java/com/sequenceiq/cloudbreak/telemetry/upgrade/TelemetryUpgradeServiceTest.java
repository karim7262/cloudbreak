...
public class TelemetryUpgradeServiceTest {
    ...
    public void testUpdateTelemetryComponent() throws Exception {
        // GIVEN
        given(orchestratorMetadataProvider.getOrchestratorMetadata(STACK_ID)).willReturn(metadata);
        given(telemetryUpgradeConfiguration.getDesiredCdpTelemetryVersion()).willReturn("0.1.0");
        ...
        // THEN
        verify(telemetryUpgradeConfiguration, times(1)).getDesiredCdpTelemetryVersion();
    ...
    public void testUpdateLoggingAgentComponent() throws Exception {
        // GIVEN
        given(orchestratorMetadataProvider.getOrchestratorMetadata(STACK_ID)).willReturn(metadata);
        given(telemetryUpgradeConfiguration.getDesiredCdpLoggingAgentVersion()).willReturn("0.1.0");
        doNothing().when(telemetryOrchestrator).updateTelemetryComponent(anyList(), anySet(), any(), anyMap());
        // WHEN
        underTest.upgradeTelemetryComponent(STACK_ID, TelemetryComponentType.CDP_LOGGING_AGENT, null);
        // THEN
        verify(telemetryUpgradeConfiguration, times(1)).getDesiredCdpLoggingAgentVersion();
        verify(orchestratorMetadataProvider, times(1)).getOrchestratorMetadata(STACK_ID);
        verify(telemetryOrchestrator, times(1))
                .updateTelemetryComponent(anyList(), anySet(), any(), anyMap());
    }
    ...
    public void testUpdateMeteringComponent() throws Exception {
        // GIVEN
        given(orchestratorMetadataProvider.getOrchestratorMetadata(STACK_ID)).willReturn(metadata);
        given(telemetryUpgradeConfiguration.getDesiredMeteringAgentDate()).willReturn("2000-01-01");
        ...
        // THEN
        verify(telemetryUpgradeConfiguration, times(1)).getDesiredMeteringAgentDate();
        verify(orchestratorMetadataProvider, times(1)).getOrchestratorMetadata(STACK_ID);
    ...
    public void testUpgradeTelemetrySaltPillars() throws Exception {
        ...
        doNothing().when(hostOrchestrator).initSaltConfig(anyList(), anySet(), any(), any());
        // WHEN
        underTest.upgradeTelemetrySaltPillars(STACK_ID, componentTypes);
        // THEN
        verify(hostOrchestrator, times(1)).initSaltConfig(anyList(), anySet(), any(), any());
    }
