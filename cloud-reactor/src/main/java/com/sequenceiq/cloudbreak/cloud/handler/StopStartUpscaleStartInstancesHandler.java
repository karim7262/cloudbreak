    ...
    private List<CloudVmInstanceStatus> startInstances(
        ...
        try {
            ...
            if (originalException instanceof PollerStoppedException) {
                LOGGER.warn("Timed out while attempting to start instances. Attempting to get states for attempted nodes", originalException);
            } else {
                LOGGER.warn("Exception while attempting to start instances. Attempting to get states for attempted nodes", originalException);
            }
            try {
                return connector.instances().checkWithoutRetry(ac, instancesToStart);
            } catch (Exception e) {
                LOGGER.warn("Error while trying to get instance status after start failure. Propagating original error with the start attempt", e);
                // The message in case of PollerStoppedException is far too verbose for the console.
